...
Positional Arguments:
  {merge,install,install-all,conda-lock,pixi,pip-compile,pip,conda,version}
    ...
    conda-lock          Generate a global `conda-lock.yml` file for a
                        ...
                        `conda-lock.yml` files for each `requirements.yaml` or
                        `pyproject.toml` file consistent with the global lock
                        file.
    pixi                Pixi integration commands for generating pixi.toml and
                        lock files.
...
Options:
  ...
  -f, --conda-lock-file CONDA_LOCK_FILE
                        ...
                        contains all dependencies. Must be used with
                        `--conda-env-name` or `--conda-env-prefix`.
...
Use `--only-global` to generate only the global lock file. The
`--check-input-hash` option can be used to avoid regenerating lock files if
...
`requirements.yaml` or `pyproject.toml` files using `pip-compile` from
`pip-tools`. This command consolidates all pip dependencies defined in the
...
Positional Arguments:
  extra_flags           Extra flags to pass to `pip-compile`. These flags are
                        ...
                        expected by `pip-compile`. For example, `unidep
                        pip-compile -- --generate-hashes --allow-unsafe`. Note
