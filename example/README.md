...
```bash
$ unidep install-all -e --dry-run
📦 Installing conda dependencies with `conda install --yes --override-channels --channel conda-forge adaptive-scheduler numpy">=1.21" hpc05 pandas">=1,<3" pexpect libzlib adaptive">=0.15.0, <2.0.0" pfapack packaging pytest pytest-cov`

📦 Installing pip dependencies with `/opt/hostedtoolcache/Python/3.12.6/x64/bin/python -m pip install unidep yaml2bib rsync-time-machine slurm-usage pyyaml aiokef markdown-code-runner numthreads`
